...
const usersBackendURL = "http://localhost:2000/seller";
...
class SellerLoginComponent extends Component {
  ...
  submitSignIn = (e) => {
    e.preventDefault();
    const userLoginData = { sEmail: this.state.form.email, sPass: this.state.form.password };
    ...
    // let emailCust=JSON.parse(bytes.toString(CryptoJS.enc.Utf8));
    // if(emailCust==this.state.form.email){
      axios.post(usersBackendURL + '/sellerLogin', userLoginData)
      .then(response => {
        this.setState({ successResponse: response.data });
      })
      .catch(error => {
    ...
    // }
    // else{
    //   toast("account credentials not matching..pls try the same email id of your account");
  ...
  crypt=(data) =>{
    ...
    sessionStorage.setItem("email_s", ciphertext);
    toast("Successfully logged in!");
  }
  sessionStore=()=>{
    //seller gurthinchaaliga
    sessionStorage.setItem("user_s",this.state.successResponse.sProfile.sName)  
  } 
  render() {
    ...
    if(this.state.successResponse){
      ...
      window.location.href="http://localhost:3000/sellerProduct"
        ...
        <div className="col-md-4 offset-4">
          <main className={classes.main}>
            ...
            <Paper className={classes.paper}>
              ...
              <Link to="/sellerRegister" exact={"true"} onClick={this.handleClick}>Don't have an account? Create One!</Link><br />
...
export default withStyles(styles)(SellerLoginComponent)
